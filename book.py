        ...
        if error is None:
            try:
                if id_data == "0":
                    ...
                    flash(
                        book_model.update(
                            title, isbn13, qty, author_selected_list, id_data
                        )
                    )
            ...
            return render_template(
                "add_update_book.html",
                res={
                    "id_data": id_data,
                    "author_list": author_list,
                    "author_selected_list": author_selected_list,
    ...
    if request.method == "POST":
        ...
        if error is None:
            try:
                ...
                while i < bookCnt:
                    url = (
                        "https://frappe.io/api/method/frappe-library?title="
                        + title
                        + "&page="
                        + str(page)
                    )
            ...
            flash(error)
            return render_template(
                "import_api_book.html", res={"title": title, "count": count}
            )
