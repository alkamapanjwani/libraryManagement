...
class issue_return_book_model:
    def get_issue_return_book_list(self):
        ...
        cur.execute(
            "SELECT bi.book_issue_id, b.title, m.name,GROUP_CONCAT(a.name SEPARATOR '/ ') as author,"
    ...
    def mark_return(self, id_data):
        cur = mysql.connection.cursor()
        cur.execute(
            "UPDATE book_issue SET is_returned_flag='yes'  WHERE book_issue_id=%s",
            (id_data,),
        )
    ...
    def mark_issue(self, member_id, book_id):
        cur = mysql.connection.cursor()
        cur.execute(
            "INSERT INTO book_issue (member_id, book_id) VALUES (%s, %s)",
            (member_id, book_id),
        )
        book_issue_id = cur.lastrowid
        transaction_model.insert_dr(
            member_id, Setting_model.get_book_fees(), book_issue_id, "Book Issued", cur
        )
        print(book_issue_id)
