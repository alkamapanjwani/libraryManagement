...
class transaction_model:
    def get_transaction_member_list(self, member_id):
        ...
        cur.execute(
            "SELECT transaction_id,date_inserted,comment,dr_amount,cr_amount from transaction where member_id=%s",
            (member_id,),
        )
    ...
    def insert_dr(self, member_id, dr_amount, book_issue_id, comment, cur):
        cur.execute(
            "INSERT INTO transaction (member_id, dr_amount, book_issue_id,comment) VALUES (%s, %s,%s, %s)",
    ...
    def insert_cr(self, member_id, cr_amount, comment):
        cur = mysql.connection.cursor()
        cur.execute(
            "INSERT INTO transaction (member_id, cr_amount, comment) VALUES (%s, %s, %s)",
    ...
    def get_amount_payable_member(self, member_id):
        cur = mysql.connection.cursor()
        cur.execute(
            "select ifnull((sum(dr_amount)-sum(cr_amount)),0) as amtPayable from transaction where member_id=%s",
            (member_id,),
        )
        #      " order by tr    ansaction_id desc"
        data = cur.fetchone()[0]
